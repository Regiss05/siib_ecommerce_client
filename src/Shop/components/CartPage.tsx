...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      ...
      try {
        const response = await axios.get(`${backendUrl}/cart/${user.uid}`);
    ...
    const fetchShopNames = async () => {
      ...
      await Promise.all(
        uniqueShopIds.map(async (shopId) => {
          try {
            const response = await axios.get(`${backendUrl}/shops/${shopId}`);
  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <Typography sx={{ marginBottom: "5px", textAlign: "center", fontSize: "18px", marginTop: "20px" }}>
        My Cart
      </Typography>
              ...
              <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%" }}>
                <Box sx={{ display: "flex", gap: "1px" }}>
                  <CardMedia
                    ...
                    sx={{ minWidth: 110, maxWidth: 110, height: 80 }}
                  />
                  <CardContent>
                    ...
                    >
                      <img src={iconshop} alt="shop name" />
                ...
                <Box sx={{ marginRight: "5px" }}>
                  ...
                  >
                    {item.price ? (item.price * item.quantity).toFixed(2) : "0.00"} Pi
                  </Typography>
                  <Box
                    sx={{
                      display: "flex",
